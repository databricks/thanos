	...
	for i, tcase := range []struct {
		...
		if ok := t.Run(fmt.Sprint(i), func(t *testing.T) {
			...
			for i, s := range srv.SeriesSet {
				thanostestutil.ProtoEquals(t, tcase.expected[i], labelpb.Labels(s.Labels))
...
func TestBucketStore_TimePartitioning_e2e(t *testing.T) {
	...
	for i, s := range srv.SeriesSet {
		thanostestutil.ProtoEquals(t, expectedLabels[i], labelpb.Labels(s.Labels))
