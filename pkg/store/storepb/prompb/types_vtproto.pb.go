...
func (m *Exemplar) CloneVT() *Exemplar {
	...
	if rhs := m.Labels; rhs != nil {
		...
		for k, v := range rhs {
			if vtpb, ok := interface{}(v).(interface{ CloneVT() *labelpb.Label }); ok {
				tmpContainer[k] = vtpb.CloneVT()
			} else {
				tmpContainer[k] = proto.Clone(v).(*labelpb.Label)
			}
		}
...
func (this *Exemplar) EqualVT(that *Exemplar) bool {
	...
	for i, vx := range this.Labels {
		...
		if p, q := vx, vy; p != q {
			...
			if equal, ok := interface{}(p).(interface{ EqualVT(*labelpb.Label) bool }); ok {
				if !equal.EqualVT(q) {
					return false
				}
