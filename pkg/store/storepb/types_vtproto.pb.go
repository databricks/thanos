...
func (m *Series) CloneVT() *Series {
	...
	if rhs := m.Chunks; rhs != nil {
		tmpContainer := make([]*AggrChunk, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
...
func (m *AggrChunk) CloneVT() *AggrChunk {
	if m == nil {
		return (*AggrChunk)(nil)
	}
	r := new(AggrChunk)
	r.MinTime = m.MinTime
	r.MaxTime = m.MaxTime
	r.Raw = m.Raw.CloneVT()
	r.Count = m.Count.CloneVT()
	r.Sum = m.Sum.CloneVT()
	r.Min = m.Min.CloneVT()
	r.Max = m.Max.CloneVT()
...
func (m *AggrChunk) CloneMessageVT() proto.Message {
	return m.CloneVT()
}
