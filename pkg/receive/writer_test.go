...
func TestWriter(t *testing.T) {
	...
	now := model.Now()
	lbls := labelpb.FromStrings("__name__", "test")
			...
			reqs: []*prompb.WriteRequest{
				{
					Timeseries: []*prompb.TimeSeries{
						...
						{
							Labels:  labelpb.FromStrings("__name__", ""),
							Samples: []*prompb.Sample{{Value: 1, Timestamp: 10}},
						},
					},
				},
				...
				Timeseries: []*prompb.TimeSeries{
					{
						...
						Exemplars: []*prompb.Exemplar{
							{
								Labels:    labelpb.FromStrings("traceID", "123"),
								Value:     111,
								Timestamp: 11,
							},
							{
								Labels:    labelpb.FromStrings("traceID", "234"),
								Value:     112,
								Timestamp: 12,
							},
						},
					...
					Timeseries: []*prompb.TimeSeries{
						{
							...
							Exemplars: []*prompb.Exemplar{
								{
									Labels:    labelpb.FromStrings("traceID", "1234"),
									Value:     111,
									Timestamp: 10,
								},
							},
					...
					Timeseries: []*prompb.TimeSeries{
						{
							...
							Exemplars: []*prompb.Exemplar{
								{
									Labels:    labelpb.FromStrings(strings.Repeat("a", exemplar.ExemplarMaxLabelSetLength), "1"),
									Value:     111,
									Timestamp: 11,
								},
							},
...
func assertWrittenData(t *testing.T, app Appendable, expectedIngested []*prompb.TimeSeries) {
	...
	for _, ts := range expectedIngested {
		l := labelpb.ToPromLabels(ts.Labels)
		hash := l.Hash()
		ref, _ := gr.GetRef(l, hash)
		testutil.Assert(t, ref != 0, fmt.Sprintf("appender should have reference to series %v, hash: %d", ts, hash))
	}
}
...
func generateLabelsAndSeries(numLabels int, numSeries int, generateHistograms bool) []*prompb.TimeSeries {
	...
	l := make(labelpb.Labels, 0, numLabels)
