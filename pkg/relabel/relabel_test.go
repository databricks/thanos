...
func TestRelabel(t *testing.T) {
	tests := []struct {
		input   labelpb.Labels
		relabel []*relabel.Config
		output  labelpb.Labels
		drop    bool
	}{
		{
			input: labelpb.FromMap(map[string]string{
				"a": "foo",
				"b": "bar",
				"c": "baz",
			}),
			...
			output: labelpb.FromMap(map[string]string{
				"a": "foo",
				"b": "bar",
				"c": "baz",
				"d": "choo-choo",
	...
	for i, test := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			if keep {
				require.Truef(t, labelpb.Equal(test.output, res),
					"expected %s but got %s", test.output, res)
			}
		})
	}
}
	...
	tests := []struct {
	...
	}{
		{
            ...
            - regex:
              replacement: static
              target_label: abc`,
			lbls: labelpb.FromStrings("__meta_dns_name", "example-some-x.com", "abc", "def", "job", "foo"),
		},
		{
			name: "kubernetes",
			config: `
