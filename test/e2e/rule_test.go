...
func rulegroupCorrectData(t *testing.T, ctx context.Context, endpoint string) {
	...
	for _, g := range data.Data.Groups {
		...
		testutil.Assert(t, g.LastEvaluation != nil && !g.LastEvaluation.AsTime().IsZero(), "expected the rule group to be evaluated at least once")
	}
}
	...
	err = runutil.Retry(time.Second*1, ctx.Done(), func() error {
		...
		if len(alerts) > 0 {
			if alerts[0].State == rulespb.AlertState_FIRING {
				...
				if alertActiveAt.AsTime().Unix() == alerts[0].ActiveAt.AsTime().Unix() {
					return nil
				} else {
